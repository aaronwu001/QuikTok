...
def showparticipants():
    user_id = request.args.get('user_id')
    return render_template('showparticipants.html', user_id=user_id)
...
def on_the_way():
    ...
    return render_template('ontheway.html', user_id=user_id, talk_title=talk_title)


@app.route("/finalpage")
def finalpage():
    users = []
    return render_template("finalpage.html", users=users)
...
@app.route('/get-attendees', methods=['GET', 'POST'])
def getAttendees():
    data = request.json  # Get the JSON data sent from the client
    user_id = data.get('user_id')
    talk_id = db.get_talk_by_user(user_id)
    attendees = db.get_all_attendees(str(talk_id))
    if attendees:
